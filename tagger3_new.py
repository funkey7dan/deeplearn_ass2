...
class Tagger(nn.Module):
    def __init__(self, vocab, labels_vocab, embedding_matrix, pre_embedding_matrix, suf_embedding_matrix):
        """
...
def read_data(
        fname, embedding_vecs=None, pre_vocab=None, suf_vocab=None, window_size=2, vocab=None, labels_vocab=None, type="train", task=None, pretrained_words_vocab=None
):
    """
        ...
        for i in range(len(tokens)):
            ...
            if i < window_size:
                for j in range(window_size - i):
                    window.append((pre_words_idx_dict["PAD"], tokens_to_idx["PAD"], suf_words_idx_dict["PAD"]))
            extra_words = tokens[max(0, i - window_size):min(len(tokens), i + window_size + 1)]
            window_tuples = []
            for word in extra_words:
                pre_in_tuple, word_in_tuple, suf_in_tuple = pre_words_idx_dict["UNK"], tokens_to_idx["UNK"], \
    ...
    return torch.tensor(labels_idx), windows, vocab, labels_vocab, labels_to_idx, all_tokens, embedding_vecs, pre_vocab, suf_vocab
...
def main_pretrained(task="ner"):
    ...
    labels_idx, windows, vocab, labels_vocab, labels_to_idx, _, embedding_vecs, pre_vocab, suf_vocab =\
        read_data(embedding_vecs=embedding_vecs,
                  fname=f"./{task}/train", task=task,
                  type="train", pretrained_words_vocab=words_embedding_vocabulary)

    labels_idx_dev, windows_dev, _, _, _, _, embedding_vecs, pre_vocab, suf_vocab =\
        read_data(pretrained_words_vocab=words_embedding_vocabulary,
                  embedding_vecs=embedding_vecs, vocab=vocab,
                  labels_vocab=labels_vocab, fname=f"./{task}/dev",
                  task=task, type="dev", pre_vocab=pre_vocab, suf_vocab=suf_vocab)
    ...
    pre_embedding_matrix = nn.Embedding(len(pre_vocab), 50)
    ...
    model = Tagger(vocab, labels_vocab, embedding_matrix, pre_embedding_matrix=pre_embedding_matrix,
    ...
    dev_loss, dev_accuracy, dev_accuracy_no_o = train_model(model, input_data=dataset,
        dev_data=dev_dataset, epochs=3, lr=0.0008, labels_to_idx=labels_to_idx, task=task)

    # plot the results
    plot_results(dev_loss, dev_accuracy, dev_accuracy_no_o, task)

    print("Test")
    _, windows_test, _, _, _, original_words, embedding_vecs, pre_vocab, suf_vocab = \
        read_data(embedding_vecs=embedding_vecs, fname=f"./{task}/test", vocab=vocab, labels_vocab=labels_vocab,
                         type="test", task=task, pre_vocab=pre_vocab, suf_vocab=suf_vocab)
...
def main_not_pretrained(task="ner"):
    labels_idx, windows, vocab, labels_vocab, labels_to_idx, _, _, pre_vocab, suf_vocab = \
        read_data(fname=f"./{task}/train", task=task,
                  type="train")

    labels_idx_dev, windows_dev, _, _, _, _, _, _, _ = \
        read_data(vocab=vocab,labels_vocab=labels_vocab, fname=f"./{task}/dev",
                  task=task, type="dev", pre_vocab=pre_vocab, suf_vocab=suf_vocab)
    ...
    # train model
    dev_loss, dev_accuracy, dev_accuracy_no_o = train_model(model, input_data=dataset,
                                                            dev_data=dev_dataset, epochs=1, lr=0.0008,
                                                            labels_to_idx=labels_to_idx, task=task)

    # plot the results
    plot_results(dev_loss, dev_accuracy, dev_accuracy_no_o, task)

    print("Test")
    _, windows_test, _, _, _, original_words, embedding_vecs, pre_vocab, sub_vocab = \
        read_data(fname=f"./{task}/test", vocab=vocab, labels_vocab=labels_vocab,
                  type="test", task=task, suf_vocab=suf_vocab, pre_vocab=pre_vocab)
